...
def calculate_demographic_data(print_data=True):
    ...
    countries_by_earnings = df[rich].groupby("native-country").size().reset_index()
    countries_by_earnings.rename(columns={0: "rich-count"}, inplace=True)
    ...
    occupations = df[rich & (df["native-country"] == "India")].groupby("occupation").size().reset_index()
    occupations.rename(columns={0: "count"}, inplace=True)
